...
// ആവശ്യമായ എല്ലാ ഐക്കണുകളും ഇവിടെ ഇമ്പോർട്ട് ചെയ്തിട്ടുണ്ട്
import { 
  Star, ChevronDown, ChevronUp, Heart, Search, ArrowRight, 
...
export default function ProductPage() {
  ...
  const [filteredProducts, setFilteredProducts] = useState([]); // ഫിൽട്ടർ ചെയ്ത ലിസ്റ്റ് സൂക്ഷിക്കാൻ
  ...
  useEffect(() => {
    ...
    if (activeCategory !== "All") {
      // നിങ്ങളുടെ API-ൽ category ഉണ്ടെങ്കിൽ ഇത് വർക്ക് ചെയ്യും
    ...
    <div className="min-h-screen bg-[#f5f5f7] pt-24 font-['DM_Sans']">
      ...
      <div className="max-w-[1400px] mx-auto p-4 mb-2 flex justify-center">
        <div className="relative w-full max-w-xl">
          ...
          />
          <Search className="absolute left-4 top-1/2 translate-y-[-50%] text-gray-400" size={18} />
        </div>
      </div>

      <div className="max-w-[1400px] mx-auto flex flex-col md:flex-row gap-4 p-4 mb-20">
        
        {/* LEFT SIDE: FILTERS (UPDATED WITH ICONS & LOGIC) */}
        ...
        <div className="flex-1 bg-white rounded-[2.5rem] shadow-sm border border-gray-100 overflow-hidden">
          {filteredProducts.length > 0 ? (
            filteredProducts.map((p) => (
              <div key={p.id} className="flex flex-col md:flex-row p-8 border-b border-gray-100 hover:bg-gray-50 transition-colors gap-8 cursor-pointer group m-2 rounded-2xl">
                <div className="w-full md:w-[200px] h-[200px] flex justify-center items-center relative rounded-3xl p-4">
                  <Heart className="absolute top-2 right-2 text-gray-400 group-hover:text-rose-600 transition-colors" size={18} />
                  <img 
                ...
                <div className="flex-1">
                  <h2 className="text-xl font-bold group-hover:text-rose-600 mb-3 text-black transition-colors">{p.name}</h2>
                  <div className="flex items-center gap-2 mb-4">
                    <span className="bg-green-700 text-white text-[11px] px-2.5 py-1 rounded-full flex items-center gap-1 font-bold">
                      4.7 <Star size={10} fill="white"/>
                    </span>
                    <span className="text-gray-500 text-[11px] font-bold">910 Ratings</span>
                  </div>
                  <p className="text-xs text-gray-600 leading-relaxed">{p.description?.substring(0, 150)}...</p>
                </div>
                <div className="w-full md:w-[220px] flex flex-col justify-center items-start md:items-end">
                  <span className="text-3xl font-black text-black mb-4">₹{p.price}</span>
                  <button className="bg-rose-600 text-white px-8 py-3 rounded-full text-xs font-bold uppercase hover:bg-rose-700 transition-all">
                    View Details
                  </button>
                </div>
              </div>
